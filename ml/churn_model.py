...
def train_churn_model(features: pd.DataFrame) -> dict:
    ...
    if backend == "xgboost":
        ...
        feature_importance.to_csv(MODEL_DIR / "churn_feature_importance.csv", header=["importance"])
        scaler = None
    else:
    ...
    return {"model": model, "scaler": scaler, "metrics": {"auc": auc, "report": report}, "backend": backend, "scored": df}
